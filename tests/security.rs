...
fn a_session_owner_may_act_on_it_but_a_stranger_may_not() {
    ...
    let id = mgr
        .create_session(fake_user(real_uid, &tmp.path().join("home")), "seat0", SessionType::Wayland, &settings)
        .unwrap();
...
fn only_root_or_the_account_itself_may_start_its_session() {
    ...
    let request = Request::CreateSession { user_name: "root".into(), seat_id: None, session_type: None };
