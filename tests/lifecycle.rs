...
fn begin_creates_runtime_dir_in_starting_state() {
    ...
    let settings = SessionSettings { runtime_dir: tmp.path().join("run"), ..Default::default() };
    ...
    let runtime_dir = ctx.environment.get("XDG_RUNTIME_DIR").expect("XDG_RUNTIME_DIR should be set");
    assert!(Path::new(runtime_dir).is_dir(), "runtime dir should have been created on disk");
}
...
fn state_machine_enforces_the_documented_transitions() {
    ...
    assert!(SessionState::Active.transition(SessionState::Closing).is_ok());
