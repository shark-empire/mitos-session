...
impl Daemon {
    ...
    fn handle_ipc(&mut self, msg: ipc::ManagerMessage) {
        match msg {
            ...
            ipc::ManagerMessage::Disconnected { conn_id } => {
                ...
                // If this connection was a session's registered
                // compositor, it just lost its display. A full
                // implementation would flag the session for
                // relaunch-with-backoff via `launcher`; left as a
                // follow-up hook here (see README roadmap).
    ...
    fn on_signal(&mut self, event: signals::SignalEvent) {
        match event {
            ...
            signals::SignalEvent::ReapChildren => reap_children(),
        }
    }
}
...
/// Reap every exited child without blocking. Compositors and autostart
/// applications are the only children this daemon ever forks, so any
/// `SIGCHLD` means one of those exited.
fn reap_children() {
    ...
    loop {
        match waitpid(Pid::from_raw(-1), Some(WaitPidFlag::WNOHANG)) {
            ...
            Ok(status) => tracing::debug!(?status, "reaped child process"),
        }
    }
}
